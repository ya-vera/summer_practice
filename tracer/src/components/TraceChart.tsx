...
const TraceChart: Component = () => {
  ...
  onMount(() => {
    ...
    const x = d3.scaleLinear().domain([0, arr.length-1]).range([0, width]);
    const y = d3.scaleLinear().domain([d3.min(arr)!, d3.max(arr)!]).range([height, 0]);
    const line = d3.line<number>().x((_,i)=>x(i)).y(v=>y(v));
    svg.append('path').datum(arr).attr('fill','none').attr('stroke','steelblue').attr('d',line as any);
  });
