...
const BoxplotChart: Component = () => {
  ...
  createEffect(() => {
    ...
    const x = d3.scaleBand().domain([state.selected[0]]).range([margin.left, width - margin.right]).padding(0.1);
    const y = d3.scaleLinear()
      .domain([Math.min(whiskerMin, d3.min(arr)!), Math.max(whiskerMax, d3.max(arr)!)]).nice()
